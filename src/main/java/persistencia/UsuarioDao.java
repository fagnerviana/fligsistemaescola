...
public class UsuarioDao {
	...
	/*
	 * public void updatePessoa(Pessoa pessoa) { try { PreparedStatement statement =
	 * connection.prepareStatement(
	 * "UPDATE pessoa SET nome = ?,email=?,renda=? WHERE id_pessoa=?");
	 * //statement.setInt(1,pessoa.getId()); statement.setString(1,pessoa.getNome()
	 * ); statement.setString(2,pessoa.getEmail()); statement.setDouble(3,
	 * pessoa.getRenda()); statement.setInt(4, pessoa.getId());
	 * 
	 * statement.execute(); statement.close(); } catch (SQLException e) {
	 * e.printStackTrace(); } }
	 * 
	 * public void deletePessoa(int id) { try { PreparedStatement statement =
	 * connection.prepareStatement( "DELETE from pessoa WHERE id_pessoa=?");
	 * statement.setInt(1,id); statement.executeUpdate(); } catch (SQLException e) {
	 * e.printStackTrace(); } }
	 * 
